    ...
    * >>>
    ...
    def create_edges(self):
        ...
        for i, (u, v, data) in enumerate(self.graph.edges(data=True)):
            ...
            plot_edge = {'u': u, 'v': v, 'label': label, 'x': time, 'y': j * step, 'start': time, 'end': time + duration}
    ...
    def draw_plotly(self):
        ...
        edge_trace = go.Scatter(
            ...
            name='Edges',
            hoverinfo='text',
            hovertext=[f"Edge: {edge['u']} -> {edge['v']}<br>Label: {edge['label']}<br>Time: {edge['x']}" for edge in self.edges]
        )

        fig.add_trace(edge_trace, row=1, col=1)

        table_header = dict(values=["Inserted Node", "Linked Node", "Loopback", "Node -- Node connections"], align='left')
        ...
        for t in range(max(edge['time_index'] for edge in self.edges) + 1):
            frame_data = [
                go.Scatter(x=[edge['x'] for edge in self.edges if edge['time_index'] <= t],
                           y=[edge['y'] for edge in self.edges if edge['time_index'] <= t],
                           mode='markers+text' if show_labels else 'markers',
                           marker=dict(color=[edge['y'] for edge in self.edges if edge['time_index'] <= t],
                                       colorscale='Viridis', size=10),
                           text=[f"{edge['u']}-{edge['v']}" for edge in self.edges if edge['time_index'] <= t] if show_labels else None,
                           textposition="top center",
                           hoverinfo='text',
                           hovertext=[f"Edge: {edge['u']} -> {edge['v']}<br>Label: {edge['label']}<br>Time: {edge['x']}" for edge in self.edges if edge['time_index'] <= t]
                )
            ]
            ...
            for i, (u, v, label) in enumerate(self.connections):
                edge = next(edge for edge in self.edges if edge['u'] == u and edge['v'] == v)
                if edge['time_index'] <= t:
                    try:
                        x_start = edge['x']
                        y_start = edge['y']
                        x_end = next(edge['x'] for edge in self.edges if edge['u'] == v and edge['time_index'] <= t)
                        y_end = next(edge['y'] for edge in self.edges if edge['u'] == v and edge['time_index'] <= t)
                    ...
                    if edge['time_index'] == t:  # Add the shape only in the frame it is introduced
                        if x_end < x_start:
                            arc_shape, arc_hover_trace = self.draw_arc(x_start, y_start, x_end, y_end, u, v)
            ...
            for col in range(4):
                table_cells['values'][col] = (current_connections[col] + table_cells['values'][col])[:20]

            frame_data.append(go.Table(header=table_header, cells=dict(values=table_cells['values'], align='left')))

            frames.append(go.Frame(data=frame_data + hover_traces, name=str(t), layout=dict(shapes=current_shapes)))
        ...
        fig.update_layout(
            ...
            xaxis=dict(range=[0, max_x + 5]),  # Set a larger initial range for the x-axis
            ...
            sliders=[{
                'steps': [{'label': str(t), 'method': 'animate', 'args': [[str(t)], {'frame': {'duration': 500, 'redraw': True}, 'mode': 'immediate'}]} for t in range(max(edge['time_index'] for edge in self.edges) + 1)],
