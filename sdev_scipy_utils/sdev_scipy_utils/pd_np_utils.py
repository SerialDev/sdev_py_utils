...
def pd_series_from_base64(encoded):
    return pd.DataFrame(list(json.loads(base64.b64decode(encoded.encode("ascii"))).values()),
                        columns="Values")
...
def np_parallel(func, data, parts=4, verbose=False):
    ...
    if Pool.__module__ == 'multiprocessing.dummy':
        if verbose:
            print('executing {func} using multiple({parts}) threads'.format(func = func, parts = parts))
            return np_multi(func, data, parts)
        else:
            return np_multi(func, data, parts)
    elif Pool.__module__ == 'multiprocessing':
        if verbose:
            print('executing {func} using multiple({parts}) processes'.format(func = func, parts = parts))
            return np_multi(func, data, parts)
        else:
            return np_multi(func, data, parts)
...
def get_between(data, col=None, _from=None, _to=None):
    ...
    def pd_get_between(df, col=col, _from=None, _to=None):
        if _from == None and _to == None:
            return df[(df['{}'.format(col)] >= df['{}'.format(col)].min()) & (df['{}'.format(col)] <= df['{}'.format(col)].max())]
        elif _from == None and _to is not None:
            return df[(df['{}'.format(col)] >= df['{}'.format(col)].min()) & (df['{}'.format(col)] <= _to)]
        elif _from is not None and _to == None:
            return df[(df['{}'.format(col)] >= _from) & (df['{}'.format(col)] <= df['{}'.format(col)].max())]
        else:
...
def timed_slice(data, timeseries= None, weeks=0, days=0, hours=0, minutes=0, seconds=0, milliseconds=0, microseconds=0  ):
   ...
   pd.DataFrame|np.array
    ...
    def np_timed_slice(ndarray, weeks=0, days=0, hours=0, minutes=0, seconds=0, milliseconds=0, microseconds=0  ):
        ...
        range_min = (((((((range_max - _weeks) - _days) - _hours) - _minutes) - _seconds) - _milliseconds) - _microseconds)
        sliced_df =  ndarray[(ndarray >= range_min) & (ndarray <= range_max)]
        return sliced_df

    def pd_timed_slice(df,timeseries, week=0, day=0, hour=0, minute=0, second=0, millisecond=0, microsecond=0  ):
        df[timeseries] = pd.to_datetime(df[timeseries])
        range_max = df[timeseries].max()
        range_min = range_max - datetime.timedelta(weeks= week, days=day, hours=hour, minutes=minute, seconds=second, milliseconds=millisecond, microseconds= microsecond )
    ...
    if type(data) == pd.core.frame.DataFrame:
        ...
        return pd_timed_slice(data, timeseries, weeks, days, hours, minutes, seconds, milliseconds, microseconds )

    if type(data) == np.ndarray:
        return np_timed_slice(data, weeks, days, hours, minutes, seconds, milliseconds, microseconds)
...
def get_subset(data, col=None, cond_value=0, flag=None):
    ...
    def np_get_subset(array, cond_value, flag=None):
        if flag == '==':
            ...
            return array
    if type(data) == pd.core.frame.DataFrame:
        assert col is not None, 'Data type{} : please provide column name'.format(type(data))
        return pd_get_subset(data, col, cond_value, flag)
    elif type(data) == np.ndarray:
        return np_get_subset(data, cond_value, flag)
    else:
        raise TypeError('Please provide a Pandas DataFrame or a numpy array, instead of {}'.format(type(data)) )
...
def get_top_correlations(df, column, threshold=0.65, top=3):
    column_corr = np.fabs(df.corr()[column].drop(column)).sort_values(ascending=False,
                                                                      inplace=False)
    top_corr = column_corr[(column_corr > threshold)][:top].index
    correlations = df.corr()[column][top_corr].to_dict()
    return ', '.join('{}: {}'.format(col, _percent(val)) for col, val in correlations.items())
...
def categorical_change(df, col):
    ...
    change = cats[np.sign(np.append(0, np.diff(df['{}'.format(col)].values, 1))).astype(np.uint8) + 1]
    df['chn_{}'.format(col)] = change
    return df


def cummulative_change(df, col):
    change = df['{}'.format(col)].groupby((df['{}'.format(col)] == df['{}'.format(col)].shift()).cumsum()).cumcount()
    return change
...
def tosql(df, *args, **kargs):
   ...
   for i in range((len(df) - INITIAL_CHUNK)//CHUNKSIZE):
       ...
       lambda: df.iloc[INITIAL_CHUNK+i*CHUNKSIZE:INITIAL_CHUNK+(i
       +1)*CHUNKSIZE, :].to_sql(*args, **kargs))
       t.start()
       workers.append(t)
