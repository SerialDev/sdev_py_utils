...
def generate_setup(
    module_name, author, author_email, description, version="0.1", url="", license="MIT"
):
...
def generate_importables(module_name):
    with open(
        os.path.join(os.getcwd(), module_name, module_name, "__init__.py"), "w"
    ) as f:
        f.write("from .example_module import *")

    with open(
        os.path.join(os.getcwd(), module_name, module_name, "example_module.py"), "w"
    ) as f:
