...
def get_all(mod):
        print('----{functions}-----\n')
        try: print(inspect_module(mod, 'functions'))
        except Exception as e: print(e)
        print('----{method}--------\n')
        try: print(inspect_module(mod, 'method'))
        except Exception as e: print(e)
        print('----{attributes}----\n')
        try: print(inspect_module(mod, 'attributes'))
        except Exception as e: print(e)
        print('----{signature}-----\n')
        try: print(inspect_module(mod, 'signature'))
        except Exception as e: print(e)
        print('----{fun_types}-----\n')
        try: print(inspect_module(mod, 'fun_types'))
        except Exception as e: print(e)
        print('--{parent_module}---\n')
        try: print(inspect_module(mod, 'parent_module'))
        except Exception as e: print(e)
        print('----{types}----------\n')
        try: print(inspect_module(mod, 'types'))
        except Exception as e: print(e)

def inspect_types(mod):
    AsyncGeneratorType, BuiltinFunctionType, BuiltinMethodType, CodeType = [[] for i in range(4)]
    CoroutineType, DynamicClassAttribute, FrameType, FunctionType = [[] for i in range(4)]
    GeneratorType, GetSetDescriptorType, LambdaType, MappingProxyType = [[] for i in range(4)]
    MemberDescriptorType, MethodType, ModuleType, SimpleNamespace = [[] for i in range(4)]
    TracebackType = []

    for key, value in mod.__dict__.items():
        if type(value) == types.AsyncGeneratorType: AsyncGeneratorType.append((key, value))
        if type(value) == types.BuiltinFunctionType: BuiltinFunctionType.append((key, value))
        if type(value) == types.BuiltinMethodType: BuiltinMethodType.append((key, value))
        if type(value) == types.CodeType: CodeType.append((key, value))
        if type(value) == types.CoroutineType: CoroutineType.append((key, value))
        if type(value) == types.DynamicClassAttribute: DynamicClassAttribute.append((key, value))
        if type(value) == types.FrameType: FrameType.append((key, value))
        if type(value) == types.FunctionType: FunctionType.append((key, value))
        if type(value) == types.GeneratorType: GeneratorType.append((key, value))
        if type(value) == types.GetSetDescriptorType: GetSetDescriptorType.append((key, value))
        if type(value) == types.LambdaType: LambdaType.append((key, value))
        if type(value) == types.MappingProxyType: MappingProxyType.append((key, value))
        if type(value) == types.MemberDescriptorType: MemberDescriptorType.append((key, value))
        if type(value) == types.MethodType: MethodType.append((key, value))
        if type(value) == types.ModuleType: ModuleType.append((key, value))
        if type(value) == types.SimpleNamespace: SimpleNamespace.append((key, value))
        if type(value) == types.TracebackType: TracebackType.append((key, value))
