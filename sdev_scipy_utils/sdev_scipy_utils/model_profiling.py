...
def plot_feature_importances(
    ...
    def merge_feature_prefixes(
        feature_scores, prefixes_to_merge, contains_to_merge, contains_to_ignore
    ):
        merged_scores = {}
        hover_info = {}
        feature_count = {}
        ...
        def process_features(matching_features, label):
            if not matching_features.empty:
                merged_scores[label] = matching_features.sum()
                hover_info[label] = ", ".join(matching_features.index)
                feature_count[label] = len(matching_features)
                feature_medians[label] = matching_features.median()
                top_contributors[label] = matching_features.idxmax()
                return True
            return False
        ...
        for item in contains_to_merge:
            if isinstance(item, tuple):
                if isinstance(item[0], list):
                    substrings, label = item
                    matching_features = feature_scores[
                        feature_scores.index.str.contains(
                            "|".join(substrings), case=False
                        )
                    ]
                else:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=feature_scores_df["Feature"].apply(
                lambda x: hover_info.get(x.split("[")[0], x)
            ),
            hoverinfo="text",
        )
    )
    ...
    # Apply zebra striping to y-axis labels
    ticktext = []
    for i, feature in enumerate(feature_scores_df["Feature"]):
        color = "darkblue" if i % 2 == 0 else "darkslategray"
        # Add spaces before and after the text to simulate padding
        padded_feature = f"  {feature}  "
        ticktext.append(f"<span style='color:{color};'>{padded_feature}</span>")
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            ...
            ticktext=ticktext,  # Set staggered colored labels
            showgrid=False,
